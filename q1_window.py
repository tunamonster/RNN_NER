...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for word in sentence:
            ...
            min_win, max_win = [idx - window_size, idx + window_size + 1]
            window_buffer = []
            for i in range(min_win, max_win):
                if i < 0:
                    ...
                    window_buffer += sentence[i]
            windowed_data.append(tuple([window_buffer, labels[idx]]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, [-1, Config.embed_size * Config.n_window_features ])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable("W", shape = [ Config.n_window_features * Config.embed_size, 
                                            Config.hidden_size], dtype=tf.float32, initializer = xav)
        b1 = tf.get_variable("b1", shape = [Config.hidden_size], dtype=tf.float32, initializer = xav)
        h = tf.nn.relu(tf.matmul(x, W) + b1)
        h_drop = tf.nn.dropout(h, dropout_rate)

        U = tf.get_variable("U", shape=[Config.hidden_size, 
                            Config.n_classes], dtype=tf.float32, initializer = xav)
        b2 = tf.get_variable("b2", shape=[Config.n_classes], dtype=tf.float32, initializer=xav)
