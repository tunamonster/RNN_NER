...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sentence_buffer = sentence + [zero_vector] * pad
        labels_buffer = labels + [zero_label] * pad 
        mask = [True] * len(sentence) + [False] * pad 
        ret.append(tuple([sentence_buffer, labels_buffer, mask])) 
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_loss_op(self, preds):
        ...
        unmasked_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
                                ...
                                logits = preds)
        masked_loss = tf.boolean_mask(unmasked_loss, self.mask_placeholder)
